 ...
 * Songs Table class
 */
class TempusTableSong extends JTable
{
	/**
	...
	{
		parent::__construct('#__tempus_song', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_tempus.song'));
	}	
	...
	/**
	 * Overload the store method for the Song table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('song', 'TempusTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_TEMPUS_SONG_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_tempus.song.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_tempus.song.'.(int) $this->$k;
	}

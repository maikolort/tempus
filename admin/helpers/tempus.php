...
/**
 * Tempus component helper.
 */
abstract class TempusHelper
{
	/**
	...
	**/
	...
	{
		$manifestUrl = JPATH_ADMINISTRATOR."/components/com_tempus/tempus.xml";
	...
	**/
	...
	{
		// get params
		$params	= JComponentHelper::getParams('com_tempus');
	...
	**/
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_TEMPUS_SUBMENU_DASHBOARD'), 'index.php?option=com_tempus&view=tempus', $submenu === 'tempus');
		if ($user->authorise('song.access', 'com_tempus') && $user->authorise('song.submenu', 'com_tempus'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_TEMPUS_SUBMENU_SONGS'), 'index.php?option=com_tempus&view=songs', $submenu === 'songs');
		}
		if ($user->authorise('instrument.access', 'com_tempus') && $user->authorise('instrument.submenu', 'com_tempus'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_TEMPUS_SUBMENU_INSTRUMENTS'), 'index.php?option=com_tempus&view=instruments', $submenu === 'instruments');
		}
		if ($user->authorise('instrument_type.access', 'com_tempus') && $user->authorise('instrument_type.submenu', 'com_tempus'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_TEMPUS_SUBMENU_INSTRUMENT_TYPES'), 'index.php?option=com_tempus&view=instrument_types', $submenu === 'instrument_types');
		}
	}
	 ...
	 */
	public static function xls($rows,$fileName = null,$title = null,$subjectTab = null,$creator = 'Maikol Fustes',$description = null,$category = null,$keywords = null,$modified = null)
	{
		// set the user
		$user = JFactory::getUser();
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'tempus')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'tempus', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__tempus_'.$type.' AS a');
	...
	* 
	**/
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'tempus')
	{
		// get the user object
		$user = JFactory::getUser();
		// load the JObject
	...
	**/
	public static function getModel($name, $path = JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Tempus', $config = array())
	{
		// fix the name
	...
	*/
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_tempus');
		
		$parentId = $parent->id;
		$name     = 'com_tempus.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_tempus', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__tempus_'.$table, $object, 'id');
			}
		}
		return false;
	}
